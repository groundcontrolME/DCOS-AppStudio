...
import json
import random
from faker import Factory	#fake-factory: generates good-looking random data
import datetime
...
def generate_random_number( min, max, length):
	"""
...
def get_random_for_type( my_type ):
	...
	print('**DEBUG: my_type is of type is {0}'.format(type(my_value)))

	return fake.bs() if my_type == "String"
	return generate_random_number( length=generate_random_number( length=3 ) ) \
								if my_type == "Integer"
	return generate_random_number( generate_random_number( length=7 ), \
	 							if my_type == "Long" )
	return generate_random_number( min=(-1)*generate_random_number( length=7 ) , \
								max=*generate_random_number( length=7 ) \
	 							if my_type == "Double" )
	return fake.latitude()+","fake.longitude() if type(my_value) == "Location"
	return fake.dateTime() if ( my_type == "Date/time" or my_type == "Date/Time" )
...
def generate_random_location( latitude, longitude, radius ):
	...
	print('**DEBUG: new_location is {0}'.print(new_location))
